...
func main() {
	...
	if state.IsEmpty() {
		...
		// Get LastResultsHash from block at targetHeight+1 (its header has the results hash for targetHeight).
		// This avoids loading ABCI responses which may fail if the amino codec doesn't know all event types.
		if nextMeta != nil {
